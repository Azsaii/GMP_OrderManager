...
const CouponDetailModal = ({ isVisible, onClose, coupon }) => {
  ...
  const [discountValue, setDiscountValue] = useState(
    coupon ? coupon.discountValue.toString() : ''
  ); // 할인 값 초기화
  const [discountType, setDiscountType] = useState(
    coupon ? coupon.discountType : '원'
  ); // 할인 타입 초기화
  const [minOrderValue, setMinOrderValue] = useState(
    coupon ? coupon.minOrderValue.toString() : ''
  ); // 최소 주문 금액 초기화
  const [maxDiscountValue, setMaxDiscountValue] = useState(
    coupon ? coupon.maxDiscountValue.toString() : ''
  ); // 최대 할인 금액 초기화

  // 추가 필드 상태
  const [canBeCombined, setCanBeCombined] = useState(
    coupon ? coupon.canBeCombined : true
  ); // 기본값 true
  ...
  const validateInputs = () => {
    ...
    // 최소 주문 금액 체크
    if (!minOrderValue) {
      newErrors.minOrderValue = '최소 주문 금액을 입력해야 합니다.';
    } else if (parseInt(minOrderValue) < 0) {
  ...
  const handleAddCoupon = async () => {
    ...
    try {
      await setDoc(couponRef, {
        ...
        startDate: `${startDate.getFullYear().toString().slice(-2)}${(
          startDate.getMonth() + 1
        )
          .toString()
          .padStart(2, '0')}${startDate.getDate().toString().padStart(2, '0')}`,
        endDate: `${endDate.getFullYear().toString().slice(-2)}${(
          endDate.getMonth() + 1
        )
          .toString()
      ...
      >
        <ScrollView contentContainerStyle={styles.scrollViewContent}>
          <View style={styles.modalInputContainer}>
            ...
            <Text
              style={
                errors.couponDescription ? styles.errorLabel : styles.label
              }
            >
            ...
            <Text
              style={errors.discountValue ? styles.errorLabel : styles.label}
            >
            ...
            <View style={styles.discountTypeContainer}>
              ...
              <View style={styles.discountTypeButtonContainer}>
                ...
                >
                  <Text
                    style={
                      discountType === '%' ? styles.selectedRadio : styles.radio
                    }
                  >
                    %
                  </Text>
            ...
            <Text
              style={errors.minOrderValue ? styles.errorLabel : styles.label}
            >
            ...
            <Text
              style={errors.maxDiscountValue ? styles.errorLabel : styles.label}
            >
          ...
          <View style={styles.rowContainer}>
            ...
            <View style={styles.radioContainer}>
              ...
              >
                <Text
                  style={canBeCombined ? styles.selectedRadio : styles.radio}
                >
                  가능
                </Text>
              </TouchableOpacity>
          ...
          <View style={styles.rowContainer}>
            ...
            <View style={styles.radioContainer}>
              ...
              >
                <Text style={isPublic ? styles.selectedRadio : styles.radio}>
                  공개
                </Text>
              </TouchableOpacity>
              <TouchableOpacity
          ...
          <View style={styles.rowContainer}>
            ...
            <View style={styles.radioContainer}>
              ...
              >
                <Text style={available ? styles.selectedRadio : styles.radio}>
                  활성
                </Text>
              </TouchableOpacity>
              <TouchableOpacity
            ...
            {coupon ? ( // 쿠폰이 있는 경우 수정/삭제 버튼 표시
              <>
                <View style={styles.buttonHalf}>
                  <Button
                    title="수정"
                    onPress={() => openConfirmationModal('update')}
                  />
                </View>
                <View style={styles.buttonHalf}>
                  <Button
              ...
              <View style={styles.buttonHalf}>
                <Button
                  title="추가"
                  onPress={() => openConfirmationModal('add')}
                />
              </View>
            )}
          </View>
